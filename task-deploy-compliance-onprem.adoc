...
+
--
* 참고: * Cloud Manager 3.9.10 이상을 사용하여 GCP에서 Connector를 만든 경우 모두 설정됩니다. 이전 버전을 사용하여 Connector를 생성한 경우 Connector와 연결된 GCP 서비스 계정에 다음 권한을 추가하여 Cloud Data Sense를 GCP에 배포해야 합니다.
...
| 또는 전체 명령을 미리 만들어 첫 번째 프롬프트에 'sudo./install.sh -a <account_id> -c <agent_id> -t <token>--host <ds_host>--cm-host <cm_host>--proxy-host <proxy_host>--proxy-port <proxy_port>- proxy-proxy-proxy-password- proxy -proxy user -proxy -proxy -proxy -proxy -proxy -proxy -proxy -proxy -proxy -proxy -proxy -proxy -proxy -password> 
|===
+
변수 값:
...
. 8단계에서 설명한 것처럼 설치 프로그램에서 프롬프트가 나타나면 일련의 프롬프트에 필요한 값을 입력하거나 첫 번째 프롬프트에 Complete 명령을 입력할 수 있습니다.
+
단일 호스트 설치에 사용할 수 있는 변수 외에도 새 옵션 * -n<node_ip> * 를 사용하여 스캐너 노드의 IP 주소를 지정할 수 있습니다. 여러 스캐너 노드 IP는 쉼표로 구분됩니다.

+
예를 들어, 이 명령은 scanner 노드 3개를 추가합니다. sudo./install.sh -a <account_id> -c <agent_id> -t <token>--host <DS_host>--cm-host <cm_host> * -n <node_IP1>, <node_ip2>, <node_ip2>, <node_proxy scheme-proxy -proxy -proxy -host < 프록시 포트 프록시 -proxy -proxy -proxy -proxy -proxy -proxy -proxy -proxy -proxy -proxy -proxy -proxy -port -proxy -proxy -proxy -proxy -proxy -host
