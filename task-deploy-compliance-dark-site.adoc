...
매우 안전한 Cloud Manager 설치가 필요하지만 OneDrive 계정 또는 SharePoint 계정도 스캔하려는 특수한 상황에서는 Data Sense 오프라인 설치 프로그램을 사용하여 몇 개의 선택 끝점에 대한 인터넷 액세스를 제공할 수 있습니다. 을 참조하십시오 <<SharePoint and OneDrive special requirements,SharePoint 및 OneDrive 특별 요구 사항>> 를 참조하십시오.
...
+
Cloud Manager Connector와 데이터 센스 모두 새로운 기능을 사용하기 위해 정기적인 수동 업그레이드가 필요합니다. 데이터 감지 UI 페이지 하단에 데이터 감지 버전이 표시됩니다. 를 확인하십시오 link:whats-new.html["클라우드 데이터 감지 릴리스 정보"] 각 릴리스의 새 기능 및 소프트웨어 업데이트 패키지를 사용할 수 있는 시기를 확인합니다. 그런 다음 의 단계를 수행할 수 있습니다 <<Upgrade Data Sense software,데이터 감지 소프트웨어를 업그레이드하십시오>>.
...
Data Sense를 설치하기 전에 다음 소프트웨어가 호스트에 설치되어 있어야 합니다.
...
인터넷에 액세스할 수 없는 사이트에 Cloud Manager 및 Data Sense를 배포하는 경우, 몇 개의 특정 엔드포인트에 대한 인터넷 액세스를 제공하여 SharePoint 및 OneDrive 계정의 파일을 검색할 수 있습니다.
...
----
. 설치 프로그램에서 프롬프트가 표시되면 일련의 프롬프트에 필요한 값을 입력하거나 첫 번째 프롬프트에 Complete 명령을 입력할 수 있습니다.
+
[cols="50a,50"]
|===
...
| 또는 전체 명령을 미리 만들어 첫 번째 프롬프트에 'sudo./install.sh -a <account_id> -c <agent_id> -t <token>--host <ds_host>--cm-host <cm_host>--proxy-host <proxy_host>--proxy-port <proxy_port>- proxy-proxy-proxy-password< proxy-proxy-proxy-proxy-user -proxy-password><proxy-password><agent_password><agent_password><agent_password><agent_id><agent_proxy-proxy-proxy- 
|===
+
변수 값:
...
. 9단계에서 설명한 것처럼 설치 프로그램에서 프롬프트가 나타나면 일련의 프롬프트에 필요한 값을 입력하거나 첫 번째 프롬프트에 Complete 명령을 입력할 수 있습니다.
+
단일 호스트 설치에 사용할 수 있는 변수 외에도 새 옵션 * -n<node_ip> * 를 사용하여 스캐너 노드의 IP 주소를 지정할 수 있습니다. 여러 노드 IP는 쉼표로 구분됩니다.

+
예를 들어, 이 명령은 scanner 노드 3개를 추가합니다. sudo./install.sh -a <account_id> -c <agent_id> -t <token>--host <DS_host>--cm-host <cm_host> * -n <node_IP1>, <node_ip2>, <node_ip2>, <node_proxy scheme-proxy -host 프록시 포트 프록시 프록시 프록시 -프록시 -프록시 -프록시 -프록시 -host <프록시 -프록시 -프록시 -프록시 -프록시 -프록시 -프록시 -프록시 -프록시 -프록시 -프록시 -프록시 -프록시 -프록시 -프록시 -프록시 -프록시 -프록시 -프록시 -프록시 -host
...
* 온프레미스 커넥터 소프트웨어가 최신 버전으로 업그레이드되었는지 확인합니다. https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-managing-connectors.html#upgrading-the-connector-on-prem-without-internet-access["커넥터 업그레이드 단계를 참조하십시오"^].
